...
interface IKeyBoard {
  ...
  setEqualAnimationState:React.Dispatch<React.SetStateAction<boolean>>
    setScreenState:React.Dispatch<React.SetStateAction<{
      finalValue: string;
      hasAnswer: boolean;
  ...
  screenState: {
    calculatorArray:string[], hasAnswer:boolean,
  },
}
...
export default function KeyBoard({
  ...
  const handleCalcMarkClick = useCallback((e) => {
  ...
  const handleSubtractClick = useCallback((e) => {
  ...
  const handleBackSpaceClick = useCallback(() => {
    if (hasAnswer) {
      setScreenState?.((prevState) => {
        ...
        if (lastStringLength > 1) {
          const newString = (lastString as string).slice(0, lastStringLength - 1);
